...
%
% before running this function, batch_full_session_analysis.m must have been run to create tuningdat.mat files for each subject
%%%% batch analysis also requires that, for each subject, ...
%%%%   ...........in vivo GCaMP ROI data and ex vivo M2 patch image data has been preprocessed
%
%%%%%%%%%% updated on MSI 2023-2-25
...
ops.plot_param_per_lvl2_var = 0; % run multilevel model function

ops.show_refline = 1; % show line through y=0 (except on bargraph)
    ops.refline_style = '--';
...
%% variables to test
% ops.varname = 'sf_pref';
%     ops.ylim = [0 2]; %%% for physiology supp figure violin plot
%     ops.violin_bandwidth = 0.15; %%%  for physiology supp figure violin plot
% ops.varname = 'sf_hwhm';
%     ops.ylim = [0 1.6]; %%% for physiology supp figure violin plot
...
% ops.varname = 'tf_pref';
    % ops.ylim = [0 12]; %%% for physiology supp figure violin plot
    % ops.violin_bandwidth = 0.5; %%%  for physiology supp figure violin plot
ops.varname = 'tf_hwhm';
%     ops.ylim = [0 13.5]; %%% for physiology supp figure violin plot
%     ops.violin_bandwidth = 0.5; %%% for physiology supp figure violin plot
% ops.varname = 'tf_di';
%     ops.ylim = [0.7 0.9]; 
% ops.varname = 'tf_sgnf';
...
% ops.varname = 'orient_si';
    % ops.violin_bandwidth = 0.04; %%% for violin, orient si by quant, sf/tf/orient/rspv sgnf and stim_on_rf
    % ops.ylim = [0 1.3]; %%% for violin, orient si by quant, sf/tf/orient/rspv sgnf and stim_on_rf
    % ops.ylim = [0.4 0.54]; %%% for bargraph, orient si by quant, sf/tf/orient/rspv sgnf and stim_on_rf
%     ops.ylim = [0.35 0.55]; %%% for bargraph, orient si by quant, rf_sgnf
%     ops.ylim = [0.39 0.49]; %%% for bargraph, orient si by quant, ~rf_sgnf
...
%       ops.ylim = [0.0 0.15];   %%% for pakan loc by quant, sf/tf/orient/rspv sgnf and stim_on_rf
    % ops.ylim = [-0.6 0.75]; % thesis fig 2B left.... pakan loc index
    % ops.ylim = [-0.7 0.75]; % thesis fig 2B right.... pakan loc index
%     ops.violin_bandwidth = 0.03; 
...
% ops.meets_criteria = ops.meets_criteria & [ roitable{:,'sf_sgnf'} | roitable{:,'tf_sgnf'} | roitable{:,'orient_sgnf'} | roitable{:,'rspv'} | roitable{:,'rf_sgnf'}]; %
...
if ops.show_refline && ... % show line through y=0
        any([ops.plot_violin,ops.show_boxplot,ops.show_scatter,ops.show_histograms,ops.anova_figures,ops.scatter_sessions])  
...
if ops.plot_param_per_lvl2_var
%     plot_param_per_subject()
    plot_param_per_lvl2_var()
end

%% output means of groups divided by tertile for paper fig 2a 
quantgroups = [1,2;3,4;5,6];
roitable_rows_by_tertile = [ops.meets_criteria & any(roitable.quantile == quantgroups(1,:),2),...
                            ops.meets_criteria & any(roitable.quantile == quantgroups(2,:),2),...
                            ops.meets_criteria & any(roitable.quantile == quantgroups(3,:),2)];
for igroup = 1:size(quantgroups,1)
    varvals_by_tertile{igroup} = roitable{roitable_rows_by_tertile(:,igroup),ops.varname};
    tertile_means(igroup) = mean(varvals_by_tertile{igroup});
end