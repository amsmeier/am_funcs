...
rerun_tabulate_subjects = 0; % if true, regenerate and save the table; otherwise, try to load it from disk
...
if ~rerun_tabulate_subjects
    ...
    for irun = 1:nruns
        ...
        %%% all triawise locm vs. stimparam for this run to the all-runs table
...
for ipar = op.n_stimpars
    ...
    for isub = 1:nsubs
        ...
        switch thispar
            case {'sf','tf'}
                % % % simple corr:
                parvals = tuningdat.tuningpars.locm_trials.(thispar){:,thispar}; 
                locm_vals = tuningdat.tuningpars.locm_trials.(thispar).locm_forw_mps; 
