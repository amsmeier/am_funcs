%%% address reviewer 2 rec 1 - compare overall response magnitudes in p vs ip
% first compute average overall response of each roi - pool from trials testing sf, tf, and orient

paths = setpaths_locmod(); 

%% load roitable
% load([paths.analyses, filesep, 'cells_20260615.mat']) %cells_2026 file contains roitable with resp_mean already included

%% compute mean response magnitudes and save

recompute_resp_mean = 1; 

if recompute_resp_mean % better to skip if already computed.... this loop can take a minute to get through 5k ROIs
    respmeantic = tic; 
    nrois = height(roitable)
    roitable.resp_mean = nan(nrois,1); 
    resp_mean = nan(nrois,1); 
    for iroi = 1:nrois      %  
        % .resp_mean is the mean response of this roi for each value of the parameter, e.g. 7 means for 7 sf values
        %%% we average these means to get e.g. the mean of all trials which varied sf, temp stored as e.g. 'sfmean'
    
        sf_mean_roi = mean(roitable.('sf_trials'){iroi,:}.resp_mean);
        tf_mean_roi = mean(roitable.('tf_trials'){iroi,:}.resp_mean);
        orient_mean_roi = mean(roitable.('orient_trials'){iroi,:}.resp_mean);

        % if mod(100,iroi)==0
        %     iroi
        % end

         resp_mean(iroi) = mean([sf_mean_roi; tf_mean_roi; orient_mean_roi]); % seems to go faster if we don't copy to massive roitable until the end
    end

    roitable.resp_mean = resp_mean; clear resp_mean
    

    % respmeantoc = toc(respmeantic);
    fprintf(['..... Took ', num2str(respmeantoc), 'sec to compute resp_mean'])
    % tic; save([paths.analyses, filesep,'cells_.mat'],'roitable', '-v7.3'); toc
end
...
%% compare p vs. ip - code from tuning_stats script
